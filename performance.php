...
<script src="lib/gauge.min.js"></script>
		...
		if ($countyid == 0) {
           	...
           	$query_Contents =mysql_query("SELECT id, indicators as depts, FORMAT(NetEst_1,0) as allocated, FORMAT(ExchIssues_1, 0) as expended FROM  nationalexpenditure WHERE used =1");
           	} else {
           	
           	$query_Contents =mysql_query("SELECT departments.id,quarterexpenditure.AllocationQ1 as allocated , quarterexpenditure.ExpendedQ1 as expended, departments.Name as depts FROM `quarterexpenditure` inner join departments on quarterexpenditure.departmentid= departments.id where quarterexpenditure.countyid = $countyid AND quarterexpenditure.mwaka = $mwaka AND departments.viewed =1");
           	}
		$c=1;
		while($row_Contents=mysql_fetch_array($query_Contents)){
		?>

		<?php if ((int)($row_Contents['allocated']) > (int)($row_Contents['expended'])){ ?>
		...
		document.getElementById("preview_<?php echo $c?>").innerHTML = "<?php echo (int)($row_Contents['expended']/1000000);?> out of <?php echo (int)($row_Contents['allocated']/1000000);?> Mn";
		document.getElementById("dept_<?php echo $c?>").innerHTML = "<a href='data.php?countyid=<?php echo $countyid ?>&deptid=<?php echo $row_Contents['id'];?>&mwaka=<?php echo $mwaka;?>'><?php echo $row_Contents['depts'];?></a>";
		gauge<?php echo $c?>.maxValue = <?php echo (int)($row_Contents['allocated']/1000000);?>; // set max gauge value
		gauge<?php echo $c?>.animationSpeed = 32; // set animation speed (32 is default value)
		<?php  $ex = ((int)($row_Contents['expended']/1000000) == 0) ? '0.01' : (int)($row_Contents['expended']/1000000);?>
 ...
 <link rel="stylesheet" type="text/css" href="lib/fd-slider/fd-slider.css?v=2">
 <link rel="stylesheet" type="text/css" href="lib/fd-slider/fd-slider-tooltip.css">
		...
		<table class="large-12">
		  <thead>
		    ...
		    <th>&nbsp;</th>
		      ...
		      $countyid = (int)$_GET['countyid'];
		      if ($countyid==0) {
           		$result = mysql_query("SELECT indicators as 'Name of Sector', NetEst_1 as 'Net Estimates', ExchIssues_1 as 'Exchequer Issues' FROM  nationalexpenditure WHERE  used =1");
           		} else {
           		$result =mysql_query("SELECT CONCAT('<a href=data.php?countyid=$countyid&mwaka=$mwaka&deptid=',departments.id, '>',departments.Name,'</a>') as Department, quarterexpenditure.AllocationQ1 as 'Amount Allocated' , quarterexpenditure.ExpendedQ1 as 'Amount Expensed'  FROM `quarterexpenditure` inner join departments on quarterexpenditure.departmentid= departments.id where quarterexpenditure.countyid = $countyid AND quarterexpenditure.mwaka = $mwaka AND departments.viewed =1");
           		}
...
<script>
 <?php 
		...
		if ($countyid == 0) {
           	...
           	$query_Contents =mysql_query("SELECT  SUM(quarterexpenditure.AllocationQ1)/1000000 as 'netbudget', SUM(quarterexpenditure.ExpendedQ1)/1000000 as 'cob' FROM `quarterexpenditure` WHERE quarterexpenditure.countyid = $countyid AND quarterexpenditure.mwaka = $mwaka");
           	}
